...
TARGET_GROUP_ID = -1001234567890  # Target group chat ID (use negative for supergroups)
BOT_USERNAME = '@im_bakabot'  # The bot username that gives replies
BOT_USER_ID = 7974841147  # Optional: Bot's user ID (faster than username, leave None to auto-detect)
REPLY_DELAY = 3  # Delay in seconds before checking for bot reply
...
class UserbotForwarder:
    ...
    async def start(self):
        ...
        try:
            ...
            # Get bot entity with FloodWait handling
            try:
                if BOT_USER_ID:
                    logger.info(f"Using bot ID: {BOT_USER_ID}")
                    self.bot_entity = await self.client.get_entity(BOT_USER_ID)
                else:
                    logger.info(f"Resolving bot username: {BOT_USERNAME}")
                    self.bot_entity = await self.client.get_entity(BOT_USERNAME)
                    logger.info(f"Bot resolved! ID: {self.bot_entity.id} - Save this for faster startup")
                logger.info(f"Bot entity retrieved: {BOT_USERNAME}")
            except FloodWaitError as e:
                logger.error(f"FloodWait when getting bot: Need to wait {e.seconds} seconds ({e.seconds/3600:.1f} hours)")
                logger.error("Please wait and try again later, or use BOT_USER_ID instead of BOT_USERNAME")
                return False
            ...
            try:
                ...
                logger.error(f"FloodWait when getting group: Need to wait {e.seconds} seconds ({e.seconds/3600:.1f} hours)")
                return False
            except Exception as e:
                logger.error(f"Failed to get target group entity: {e}")
                return False
            
            # Test bot connection (skip if FloodWait occurred)
            logger.info("Skipping bot test message to avoid FloodWait")
            
            return True
            
        except FloodWaitError as e:
            logger.error(f"FloodWait during startup: Need to wait {e.seconds} seconds ({e.seconds/3600:.1f} hours)")
            return False
        except Exception as e:
